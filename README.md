...
| 탭 | 경로 | 내용 |
|---|---|---|
| 오늘 | `/today` | 홈 화면. 인사말·상태 메시지, 앱 바로가기 런처(추가·아이콘 등록·위젯 설정), 이번 달 요약, 오늘의 할 일 |
...
app_links                                                 (홈 앱 바로가기)
workspace_settings                                        (상태 메시지 등 환경설정)
```
...
| `GET` / `POST` | `/api/app-links` | 앱 바로가기 목록 / 추가 |
| `PATCH` / `DELETE` | `/api/app-links/:id` | 이름·링크·아이콘·표시 여부 수정 / 삭제 |
| `POST` | `/api/app-links/reorder` | 바로가기 순서 변경 |
| `POST` | `/api/app-links/resolve-icon` | 주소에서 사이트 아이콘 탐색 |
| `GET` / `PUT` | `/api/settings/status` | 프로필 상태 메시지 |
...
홈 화면에서 자주 쓰는 웹 서비스를 바로가기로 등록할 수 있습니다.

- **아이콘 우선순위**: 직접 올린 파일 → 주소에서 찾은 사이트 아이콘 → 기본 아이콘.
  아무것도 등록하지 않으면 이름에서 글자를 따 타일을 그립니다. 한글 이름이면 첫
  한글 음절을 쓰기 때문에 `Google 캘린더` 와 `Google 드라이브` 가 구분됩니다.
- **자동 탐색**: 주소 입력란에서 포커스를 빼면 해당 사이트의 `<link rel="icon">` 과
  `/favicon.ico` 를 찾아봅니다. 찾지 못해도 조용히 기본 아이콘으로 넘어갑니다.
- **위젯 설정**: 표에서 이름·링크 수정, 순서 변경, 표시 여부 토글, 삭제를 합니다.
  상태를 끈 앱은 목록에 남되 홈 화면에는 보이지 않습니다.

아이콘 탐색은 사용자가 입력한 주소로 서버가 직접 요청을 보내는 동작이라
(SSRF) 다음을 지킵니다 — http(s) 외 스킴 거부, 사설·루프백·링크로컬(클라우드
메타데이터 포함)·멀티캐스트 대역 거부, 리다이렉트마다 재검증(최대 3회),
5초 타임아웃, 본문 크기 제한, 이미지 미디어 타입만 허용.
업로드한 아이콘도 이미지 타입과 256KB 상한을 서버에서 다시 검사합니다.

> DNS 리바인딩(검사 직후 응답이 바뀌는 경우)까지는 막지 않습니다. 사내망에
> 노출되는 배포라면 아이콘 탐색을 이그레스 프록시 뒤에 두는 편이 안전합니다.
