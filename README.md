...
```bash
npm install
npm run db:seed     # data/crm.db 생성 + 데모 데이터 적재
...
SQLite 스키마는 `src/lib/db/schema.sql` 한 곳에 정의되어 있고, 앱 기동 시 자동으로 적용됩니다.
...
src/
  ...
  lib/
    ...
    types.ts          공용 도메인 타입
scripts/seed.ts       결정적 데모 데이터 생성기
```
...
`scripts/seed.ts` 는 결정적(seed 고정) 생성기입니다. 오늘 기준 27개월 구간에 대해
...
로컬 실행과 영구 디스크가 있는 호스팅(예: 컨테이너 + 볼륨)에는 그대로 사용할 수 있지만,
파일시스템이 유지되지 않는 서버리스 환경에 올릴 때는 `src/lib/db` 와 `src/lib/repositories`
를 관리형 데이터베이스(Postgres 등)로 교체해야 합니다. 스키마(`schema.sql`)와 쿼리가
한곳에 모여 있어 교체 범위는 이 두 디렉터리로 한정됩니다.

## 요구 사항

- Node.js 22.13 이상 (`node:sqlite` 사용)
