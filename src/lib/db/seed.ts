 ...
 *
 * The generator walks a 27-month timeline and produces leads, opportunities,
...
export interface SeedOptions {
  /** Fixes the dataset - the same seed always produces the same database. */
  seed?: number;
  /** "Today" for the generated timeline. Defaults to the current date. */
  now?: Date;
}
...
export function seedDatabase(db: DatabaseSync, options: SeedOptions = {}): SeedSummary {
  ...
  // ---------------------------------------------------------------------------
  const SEED = options.seed ?? Number(process.env.CRM_SEED ?? 20260829);
  ...
  // timeline
  // ---------------------------------------------------------------------------
  const NOW = options.now ?? new Date(process.env.CRM_SEED_NOW ?? new Date().toISOString());
